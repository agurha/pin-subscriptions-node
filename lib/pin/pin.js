...
const getAllBatched = function(getBatchFunction, paginationKey='pagination_id') {
	return function(sinceId, callback) {
		if(typeof sinceId === 'function'){
			callback = sinceId;
			sinceId = 0;
		}
		var batchResultsLength = 0;
		var allResults = [];
		var $this = this;
		async.doWhilst(function(cb){
			getBatchFunction.call($this, sinceId, function(err, results){
				if(err){
					console.log('errrrrr');
					return cb(err);
				}
				batchResultsLength = results.length;
				sinceId = results[results.length - 1][paginationKey];
				allResults = allResults.concat(results);
				cb(null, allResults);
			});
		},
		function(){
			return batchResultsLength === 50;
		},
...
Pin.prototype.getSubscribers = function( since_id, callback ) {
	$this = this;
	console.log('in getSubscribers', $this);
	this.$get_call( "subscribers.xml?since_id="+since_id, function( code, result ) {
		if ( Math.floor(code / 10) == 20 ) {
			var json = JSON.parse( xml2json.toJson( result ) );
			callback( null, $this.response_parser.$get_normalized_single( json.subscribers.subscriber ));
		} else if ( code == 404 ) {
			callback( { code: "subscribers_not_found", message: result }, null );
		} else {
...
Pin.prototype.getAllSubscribers = getAllBatched(Pin.prototype.getSubscribers);
...
Pin.prototype.getTransactions = function( since, callback ) {
	...
	this.$get_call( uri, function( code, result ) {
		if ( Math.floor(code / 10) == 20 ) {
			var json = JSON.parse( xml2json.toJson( result ) );
			callback( null, $this.response_parser.$get_normalized_single( json.transactions.transaction ));
		} else if ( code == 404 ) {
			callback( { code: "transactions_not_found", message: result }, null );
		} else {
			callback( { code: "error", message: result, code: code }, null );
		}
	} );
...
Pin.prototype.getAllTransactions = getAllBatched(Pin.prototype.getTransactions, 'id');
